...
/**
 * Comprehensive unit tests for SkipListMemTable.
 */
class SkipListMemTableTest {
    ...
    @Test
    void testIteratorEmptyRange() throws Exception {
        for (int i = 0; i < 10; i++) {
            memTable.put(("key" + i).getBytes(), ("value" + i).getBytes());
        }

        KVIterator it = memTable.iterator("key7".getBytes(), "key5".getBytes());
        try {
    ...
    void testByteKeys() {
        ...
        memTable.put(key1, "value1".getBytes());
        memTable.put(key2, "value2".getBytes());
        memTable.put(key3, "value3".getBytes());

        assertArrayEquals("value1".getBytes(), memTable.get(key1));
        assertArrayEquals("value2".getBytes(), memTable.get(key2));
        assertArrayEquals("value3".getBytes(), memTable.get(key3));
    }
