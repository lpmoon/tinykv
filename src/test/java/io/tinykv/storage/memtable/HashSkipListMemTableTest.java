...
/**
 * Comprehensive unit tests for HashSkipListMemTable.
 */
class HashSkipListMemTableTest {

    private HashSkipListMemTable memTable;

    @BeforeEach
    void setUp() {
        memTable = new HashSkipListMemTable();
    }
    ...
    void testCustomBucketCount() {
        HashSkipListMemTable customTable = new HashSkipListMemTable(32);

        for (int i = 0; i < 100; i++) {
            customTable.put(("key" + i).getBytes(), ("value" + i).getBytes());
        }

        for (int i = 0; i < 100; i++) {
            assertNotNull(customTable.get(("key" + i).getBytes()));
        }
    }
    ...
    @Test
    void testKeysAcrossBuckets() throws Exception {
        ...
        // Verify the keys are in lexicographic order
        List<String> expectedKeys = new ArrayList<>(expectedData.keySet());
        assertEquals(expectedKeys, seenKeys);
    }
